...
def pil_to_fig(im, showlegend=False, title=None):
    ...
    fig.add_trace(go.Scatter(
        x=[img_width * 0.0, img_width * 1.0],
        y=[img_height * 1.0, img_height * 0.0],
        showlegend=False, 
        mode="markers", 
        marker_opacity=0, 
        hoverinfo="none", 
...
def incorrect_source():
    return go.Figure().update_layout(title='incorrect source')
...
def pil_to_b64(im, enc_format='png', verbose=False, **kwargs):
    ...
    return f"data:img/{enc_format};base64, " + encoded
